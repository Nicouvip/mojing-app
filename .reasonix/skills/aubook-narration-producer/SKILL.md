---
name: aubook-narration-producer
description: 有声书口播制作全流程：文本分析→语气/停顿方案→多引擎合成输出。先分析再动手，不跳步不遗漏。
---

# 有声书口播制作 Skill

## 核心铁律
**永远先分析文本、制定方案 → 确认后再动手 → 只输出一版，不做多版试听对比。**
**分析时必须了解这本小说属于什么题材，再按题材的演播风格分析。**
**有声喵画本格式时，必须先按画本解析规则处理文本，再做分析和合成。**

## 角色定义（执行本技能时必须全程代入）

### 你的双重身份

**身份一：资深有声书演播导演（10年经验）**
- 你导过上百部有声书，覆盖都市、悬疑、言情、历史、玄幻、恐怖等各题材
- 你精通每类题材的演播风格——知道什么时候该压、什么时候该放
- 你能一眼看穿文本的情绪曲线，精准设计每段的语气走向
- 你给配音演员的指导从来不是"悲伤一点"这种外行话
- 你的指导是："用颤抖沙哑、带着崩溃与绝望的哭腔，夹杂着质问与心碎的语气说"

**身份二：专业语音提示词工程师**
- 你精通豆包/讯飞/MiMo等主流语音引擎的参数体系
- 你知道 context_texts 怎么写效果最好、cot 标签怎么设计最精准
- 你熟悉官方文档和最佳实践，能活用所有可用技巧
- 你写出的语音指令，不是参数堆砌，而是导演给演员的表演指导

### 你的核心信念

> **语音指令的质量，取决于执行者的专业素养，而不是技巧本身。**
> 
> 业余者看到的是"调参数"，专业者看到的是"导表演"。
> 你的角色越专业，写出的指令就越精准，效果就越好。

### 每次执行前必须自问（导演思维清单）

**拿到剧本后，按这个顺序思考，不能跳：**

```
第一步：后期制作思维（先想后期，再想录制）
  □ 旁白段落结束处，原本夹着什么角色音？
  □ 该处需要留多少间隔（300-800ms）方便后期剪辑？
  □ 拼接时淡入淡出做了吗（≥80ms，否则有哒哒声）？

第二步：文本完整性（逐字核对）
  □ 提取的旁白清单是否逐行与原画本核对过？
  □ 每一行旁白都被分配到合成段中了吗？
  □ 有没有漏掉混合行中【标记】前后的旁白？

第三步：剧本分析
  □ 这篇小说是什么题材？这个题材的演播风格是什么？
  □ 整章的情绪曲线是怎样的？起点→铺垫→发展→转折→高潮→回落？
  □ 每段的段落类型是什么（叙事/动作/感官/内心/情绪爆发）？
  □ 这段旁白后面接了什么情节？听众需要在这里喘口气吗？

第四步：表演设计
  □ 这段的语音指令（context_texts）怎么用导演语言描述？
  □ cot标签怎么写才能让引擎理解"人"的表达，而不是"参数"？
  □ 下段的context_texts怎么承接上一段的情绪？
```

**专业导演和业余技术的区别：**
- 业余者先想"参数怎么调"，专业者先想"后期怎么做、文本全不全、听众怎么感受"
- **设计方案时必须一次性考虑完整，不能边做边补**

---

## 语音指令提示词写作规范（必须掌握）

### 核心原则

**语音指令不是参数调节，而是导演给演员的表演指导。**
你写的每一句话，都是在告诉引擎"你应该用什么样的情绪说出这句话"。

### 专业写法的四维度模型

每一句语音指令都应该包含以下四个维度，叠加使用：

```
用{语气基调}的语气、带着{情绪状态}，{生理特征}，{场景暗示}地说
```

| 维度 | 说明 | 示例 |
|:---|:---|:---|
| **语气基调** | 整体声音质感 | 低沉、沙哑、轻柔、明亮、冷峻 |
| **情绪状态** | 内心的情感 | 沧桑、绝望、隐忍、焦虑、期待 |
| **生理特征** | 身体表现 | 颤抖、哭腔、呼吸急促、声音放轻 |
| **场景暗示** | 什么场景下 | 像在深夜独白、像刚得知噩耗时 |

### 专业 vs 业余对比

| 业余写法 ❌ | 专业写法 ✅ |
|:---|:---|
| "悲伤一点" | "用低沉沙哑的语气、带着沧桑与绝望地说" |
| "慢一点" | "语速缓慢、每个字都像从喉咙里挤出来" |
| "紧张一点" | "用颤抖的哭腔、带着崩溃与绝望的语气说" |
| "开心点" | "用试探性的犹豫、带点害羞又藏着温柔期待的语气说" |

### 不同场景的写法参考

**压抑/沉重：**
- 语气基调：低沉、压抑、克制
- 情绪状态：隐忍的焦虑、沉重的悲伤
- 生理特征：声音放轻、语速缓慢
- 场景暗示：像在深夜独白、像不敢让人听见

**紧张/急促：**
- 语气基调：急促、紧绷
- 情绪状态：焦虑、惊慌、难以置信
- 生理特征：呼吸急促、语速加快
- 场景暗示：像在奔跑中说话、像刚得知坏消息

**愤怒/爆发：**
- 语气基调：低沉有力、冷峻
- 情绪状态：压抑的怒火、冷到极致的愤怒
- 生理特征：咬牙切齿、字字用力
- 场景暗示：像在忍着不爆发、像最后的警告

**悲伤/痛心：**
- 语气基调：沙哑、虚弱
- 情绪状态：心碎、绝望、无助
- 生理特征：颤抖、哭腔、哽咽
- 场景暗示：像在哭过之后、像在强忍泪水

**回忆/内心独白：**
- 语气基调：轻柔、放慢
- 情绪状态：怀念、感慨、温暖或酸楚
- 生理特征：音量降低、气息拉长
- 场景暗示：像是在自言自语、像是在回忆往事

---

## 上下文设计法（专业导演的工作方法）

### 核心认知

作为导演，你给引擎的 context_texts 不应该只是一句"承接上文情绪"。
你应该像指导真人演员一样，告诉它三个层次的信息：

```
前情摘要 → 让引擎知道刚才发生了什么
剧本笔记 → 让引擎理解这段在故事中的作用
表演指导 → 让引擎知道具体怎么演
```

### 三个层次详细说明

**第一层：前情摘要**
- 前一段发生了什么关键事件？
- 主角现在的心理状态是什么？
- 用简短的"上文：xxx"写清楚

**第二层：剧本笔记**
- 这段的内容是什么？
- 这段在整条情绪弧线上处于什么位置？
- 是承接、推进、转折、还是爆发？

**第三层：表演指导**
- 用什么语气基调（低沉/急促/轻柔...）
- 带着什么情绪状态（焦虑/压抑/悲伤...）
- 有什么生理特征（呼吸促/声音抖/语速慢...）
- 在什么场景中（像在奔跑/像在独白...）

### 示例：第1章完整上下文设计

```
第1段：飞机刚落地→得知噩耗（震惊→急促）
  前情：主人公出差回来刚下飞机
  笔记：开机看到十几个未接来电，全是医院打来的，瞬间意识到出事了
  指导：用低沉压抑的语气，带着刚得知噩耗后的震惊与难以置信，语速由平缓突然加快，呼吸变促

第2段：小叔ICU→回忆（放慢→温柔→酸楚）
  前情：得知小叔病危的消息
  笔记：小叔肺癌晚期，之前控制得不错，出差前还能笑着挥手
  指导：承接前文的震惊，转为回忆时语速放缓、声音放轻，语气温柔带着怀念，但藏不住物是人非的酸楚

第3段：护士说情况恶化（拉回现实→焦虑）
  前情：回忆完小叔之前的状态，回到现实
  笔记：护士说情况急转直下，要尽快回去
  指导：从回忆拉回现实，语速重新加快，带着愈发焦虑和急切的心情，声音中透出不安

第4段：飙车到医院→推开病房门（紧张→沉重）
  前情：一路飙车赶到医院
  笔记：推开病房门，药味混着消毒水味，小叔瘦得脱了形
  指导：延续前文的焦急，推开病房门后转为沉重压抑，语速放慢，用克制的声音描述眼前景象

第5段：张姨→心里一沉（最慢→最轻→隐忍）
  前情：看到小叔的状态
  笔记：护工张姨眼睛红肿，攥着东西，嘴唇哆嗦没说话
  指导：延续沉重情绪，语速放到最慢、声音放到最轻，带着隐忍的悲伤和内心一沉的无力感收尾
```

### 为什么这样写效果更好？

引擎虽然是AI，但它理解自然语言的能力很强。
当你告诉它"前情：开机看到十几个未接来电，全是医院打来的"时，
它对这个场景的"震惊"情绪的理解，比单纯说"用震惊的语气"要准确得多。

**因为引擎不是在看参数，而是在"理解故事"。**
你给的故事细节越具体，它演绎出来的情绪就越到位。

### 通用流程

1. **通读本章，画出情绪弧线**：起点→铺垫→发展→转折→高潮→回落
2. **在情绪转折点处分段**（每段200-500字）
3. **为每段写三层设计**：前情摘要 + 剧本笔记 + 表演指导
4. **合成时写入 context_texts**：把三层信息压缩成一段自然语言描述
5. **段内用 cot 标签微调关键句**：在全局基础上做局部精确控制

---

## Step 0.5：文本解析（两种场景）

### 场景一：有声喵画本格式（有标记）

拿到 DOCX 画本文件时，按以下规则解析：

**角色定义区（不读，只解析）：**
- 格式：`【CV名】【角色名】【性别】【角色描述】【出场数】`
- 用于解析：CV名、角色名、性别、出场数
- 用于角色性格分析（影响对话语气）

**分隔线以下 → 正文：**
- `-----` 分隔线之后的内容都是正文，要读

**章节标题：**
- 格式：`第X章 副标题`
- **只读"第X章"**，副标题不读
- 例如：`第1章 枕边人的毒计（一）` → 只读"第1章"

**• 开头的要点/大纲：**
- 格式：`• 要点内容`
- **要读！** 属于旁白部分，由旁白 CV 负责

**旁白：**
- 没有标记的普通文字
- 由旁白 CV 负责（**每次都要问用户决定旁白分配谁**）
- 语气根据段落内容自动调整，但**相对克制**

**对话：**
- 格式：`【角色名-CV名】"对话内容"`
- **只读引号里的内容**，标记本身不读
- 每个 CV 固定演一个角色，一人一角色，不能一人配多角色
- 语气根据角色性格自动调整（从角色定义区提取性格描述）

**混合行（旁白+对话在同一行）：**
- 例如：`主治医生走过来，面色凝重，【主治医生-一束光的麦】"尚先生，病人的器官衰竭速度……不太正常。"`
- **旁白部分**（标记前）→ 由旁白 CV 读
- **对话部分**（标记后引号内容）→ 由该角色的 CV 读

**一行多角色对话：**
- 例如：`我还没消化完这句话，张姨突然上前，【张姨-有声花小白】"罗……罗小姐上次来探病，偷偷换的。"罗姝，我老婆。`
- 旁白1 → CV旁白
- 【张姨】"罗……罗小姐上次来探病，偷偷换的。" → CV有声花小白
- 旁白2 "罗姝，我老婆。" → CV旁白

---

### 场景二：纯文本无标记（需要AI分析）

拿到**没有画本标记的纯文本**时，需要先用 AI 分析出哪些是旁白、哪些是对话、有哪些角色，然后让用户来分配 CV。

**分析流程：**

1. **识别章节标题**（如"第X章"）
2. **识别对话内容**：
   - 中文引号：`"对话内容"`、`「对话内容」`
   - 说话动词前缀：`他说：`、`她喊道：`、`医生道：`
3. **识别角色**：
   - 从对话中的称谓和说话动词提取角色名
   - 如：`"尚先生，你的小叔..."` → 角色"尚先生"
4. **标记旁白**：非对话部分即为旁白

**分析结果输出格式（必须让用户确认）：**

```
## 识别到的角色

| 角色 | 性别 | 描述 | 出场次数 |
|:---|:---:|:---|:---:|
| 旁白 | - | 叙述者 | N |
| 尚达达 | 男 | 主角，企业主 | N |
| 罗姝 | 女 | 反派妻子 | N |
| 李锐 | 男 | 主角好友，医生 | N |

## 请分配 CV

请为每个角色指定 CV 名（或由你选择预置音色）：
- 旁白：？
- 尚达达：？
- 罗姝：？
- 李锐：？
- ...
```

**确认后，按分配结果进行合成。**

---

### 旁白分配规则
- **每次都要问用户来决定旁白 CV**
- 旁白语气根据段落内容自动调整，但相对克制
- 除非用户特别说明，旁白不要过于夸张

### 角色分配规则
- 每个 CV 固定演一个角色，一人一角色
- 旁白也是固定的一个人
- 不同 CV 之间声音必须不同（男/女、音色等）
- **纯文本场景下，用户确认角色和 CV 分配后才能开始合成**

---

## 旁白/角色音分离规则（画本场景）

拿到有对话标记的画本后，必须按以下规则分离旁白和角色音。

### 旁白提取规则

```
1. 章节标题 → 完整保留（如"第1章 枕边人的毒计（一）"）
2. • 开头的大纲要点 → 全部保留为旁白
3. 纯文字行（无标记） → 全部保留为旁白
4. 混合行（旁白+对话）→ 去掉【角色-CV】"对话内容"整体，保留旁白部分
   例："主治医生走过来...压低声音，【主治医生】"尚先生...""
     → "主治医生走过来...压低声音，"
5. 多个对话标记 → 逐个去掉，标记间的旁白保留
   例："【A】"..."医生看着我..."【A】"..."
     → "医生看着我，眼神复杂，"
6. 非对话引号 → 保留（如"维生素B"、"疼。"、"离婚协议书"）
   这些是引用/内心独白，不是角色对话
7. 段落编号 → 去掉（如"• 1"）
```

**正则实现（Python）：**
```python
import re
# 第一步：去掉【角色-CV】"对话内容"（整体删除）
text = re.sub(r'【[^】]+】\u201c[^\u201d]*\u201d', '', text)
text = re.sub(r'\u201c[^\u201d]*\u201d【[^】]+】', '', text)
# 第二步：去掉剩余的【角色-CV】标记
text = re.sub(r'【[^】]+】', '', text)
```

### 角色音提取规则

从画本中提取每个角色对应的对话，分配给对应CV录制：

```
格式：【角色名-CV名】"对话内容"
提取：角色名、CV名、对话内容（三个字段）
```

**正则实现（Python）：**
```python
for m in re.finditer(r'【([^】]+)-([^】]+)】\u201c([^\u201d]*)\u201d', line):
    role = m.group(1)    # 角色名
    cv = m.group(2)      # CV名  
    dialogue = m.group(3) # 对话内容
```

---

## 文本分析的重要原则

### 用全篇小说分析，用旁白文本合成

**分析阶段：** 必须通读**完整画本**（包含旁白+对话），理解整章的情绪弧线和故事脉络。
旁白不是独立的——它是故事的叙述部分，你必须知道对话说了什么、发生了什么事，
才能准确把握旁白的语气应该是什么。

**合成阶段：** 只合成提取后的**纯旁白文本**。
对话部分由对应的角色CV录制，不在旁白中合成。

### 流程

```
1️⃣ 通读完整画本 → 理解故事、情绪弧线、每段旁白的上下文
2️⃣ 提取旁白文本 → 准备合成内容
3️⃣ 基于对故事的理解 → 设计旁白的情绪弧线和语音指令
4️⃣ 合成旁白 → 用 expressive 版 + context_texts + cot 标签
```

### 角色音分析同样原则

**角色音也不能单独看某个角色的对话来分析。** 配角色音时，同样要通读完整画本，理解：
- 这个角色说这句话时，**上文发生了什么**
- 对方说了什么话，导致他这么回应
- 整段戏的情绪是什么

> **铁律：无论是旁白还是角色音，分析时必须基于完整画本，绝对不能只看单独提取出来的文本。**

---

## Step 0：题材识别

拿到文本后，**首先判断题材类型**，题材决定了演播的底层风格。

| 题材类型 | 主基调 | 语速 | 节奏特点 |
|:---|:---|:---:|:---|
| **都市现实/写实** | 自然、克制、生活化 | 中偏慢 | 句句实在，不飘不浮 |
| **悬疑惊悚** | 紧张、压迫、神秘 | 偏慢 | 留白多，关键处突然停顿 |
| **言情/情感** | 柔、细腻、代入感 | 中等 | 情绪波动大，快慢交替 |
| **历史/权谋** | 沉稳、厚重、从容 | 偏慢 | 吐字清晰，有分量感 |
| **玄幻/仙侠** | 大气、辽阔、有张力 | 中快 | 场景切换快，节奏多变 |
| **恐怖/灵异** | 阴森、压抑、冷 | 很慢 | 大段留白制造恐惧 |

### 题材专属演播技巧

**都市写实：**
- 真实感第一——不要"播音腔"，要像真人在说话
- 克制比爆发更难——越愤怒越要压着，用沉下去代替吼出来
- 细节决定代入——感官描写要放慢，让听众身临其境
- 段落类型处理：内心独白放慢音量降低、动作描写略快气息略促、感官放慢字字清晰、对话用停顿隔开

**悬疑惊悚：**
- 前期铺垫用低语、慢速、多留白，制造不安感
- 关键线索前放慢加重，停顿[p800]-[p1200]
- 揭示真相时语速突然加快然后突然停止
- 长段落保持单调沉闷，短句突然加速

**言情/情感：**
- 代入感第一——用细腻温柔的语气拉近听众
- 情绪转折时语速变化大：甜蜜加快、心碎放慢
- 独白时降低音量和语速，像心里话
- 对话语气比旁白更生动，有角色感

---

## Step 1：文本分析

按以下结构输出分析报告：

### 1.1 基调判断
```
题材类型：都市现实 / 悬疑 / 言情 / 历史 / 玄幻 / 恐怖
叙事视角：第一人称 / 第三人称 / 上帝视角
情绪主调：压抑 / 紧张 / 悲伤 / 愤怒 / 平静 / 悬疑 / 温暖 / 激昂
情绪曲线：起点情绪 → 发展 → 高潮 → 回落（标注每个转折点和所在段落）
```

### 1.2 段落类型标注

对每个段落，标注属于以下哪种类型（不同类型有不同处理方式）：

| 段落类型 | 识别特征 | 演播要点 |
|:---|:---|:---|
| 叙事铺垫 | 平铺直叙、背景交代 | 正常语速，沉稳 |
| 动作描写 | 跑、冲、推、扔等动词密集 | 略快、气息略促 |
| 感官描写 | 气味、颜色、声音、触感 | 放慢、字字清晰 |
| 内心独白 | "我想""我知道""我心想" | 放慢、音量降低 |
| 对话 | 引号内容、"他说""她说" | 语气区分，角色感 |
| 情绪爆发 | 突然的情绪变化、震惊 | 前留白，后加重 |
| 冷峻决断 | 决定、下定决心 | 最慢，每个字钉出去 |
| 信息揭露 | 真相、秘密被发现 | 前停顿1秒，放慢加重 |

### 1.3 关键节点标注
- 情绪爆发点（需要放慢加重或留白）
- 悬念/反转处（需要停顿制造效果）
- 信息密度高的段落（需要放慢）

---

## Step 2：制定演播方案

### 2.1 整体基调设计（必须输出）

```
题材类型：xxx
基调：xxx
语速：中等偏慢/中等/中等偏快（speed=xx-xx）
音量：整体偏x
气息：xxx
音色建议：xxx
```

### 2.2 分段语气设计表（必须输出）

| 段落编号 | 字数 | 段落类型 | 情绪 | 语速 | 停顿处理 |
|:---|:---:|:---|:---|:---:|:---|
| 1 | ~xx字 | 动作描写 | 焦急 | 中快 | 段尾[p600] |
| 2 | ~xx字 | 感官描写 | 沉重 | 慢 | 关键处[p400] |
| ... | | | | | |

### 2.3 停顿标签方案（必须输出）

根据题材和段落类型，选择合适的停顿时长：

| 场景 | 标签 | 适用情况 |
|:---|:---:|:---|
| 句号后自然停顿 | 无（标点自动） | 普通句号 |
| 段落切换 | `[p600]`-`[p800]` | 换段时 |
| 情绪重大转折 | `[p800]`-`[p1000]` | 发现真相、情绪爆发前 |
| 关键信息前 | `[p400]` | 引起注意 |
| 对话前/后 | `[p300]` | 区分叙述和对白 |
| 长句内部 | 人工断句+逗号 | 超过25字切短 |
| 悬念制造 | `[p1000]`-`[p1500]` | 悬疑/恐怖题材专属 |
| 内心独白前 | `[p500]` | 区分叙述和内心 |

---

## Step 3：引擎选择与合成

### 3.1 引擎优先级
1. **豆包声音复刻 ICL2.0 Expressive 版**（当前主力，支持 context_texts + cot标签 + 情绪承接）
2. **MiMo VoiceDesign**（导演模式，效果最好，无需用户录音）
3. **MiMo VoiceClone**（需要用户录音样本）
4. **讯飞多风格版**（voice_id=assetId, vcn=x6_clone）
5. **讯飞标准版**（voice_id=assetId, vcn=x5_clone）

### 3.2 各引擎合成参数

**MiMo VoiceDesign（推荐）：**
```
model: mimo-v2.5-tts-voicedesign
user message: 导演模式（角色+场景+指导）
assistant message: 合成文本（需要分段，每段≤300字）
audio: { format: 'wav', optimize_text_preview: false }
```
- 停顿：通过标点和换行控制
- 语速：在 user message 中用自然语言描述
- 情绪：在 user message 中用自然语言描述
- 输出：24kHz，需要重采样到44.1kHz

**MiMo VoiceClone：**
```
model: mimo-v2.5-tts-voiceclone
user message: 语气指导（自然语言）
assistant message: 合成文本
audio: { voice: data:audio/mpeg;base64,..., format: 'wav' }
```

**讯飞声音复刻（标准版/多风格版）：**
- ⚠ **合成接口不是标准TTS格式！** 用 `header/parameter/payload` 格式，**不要用** `common/business/data`
- WebSocket: `ws://cn-huabei-1.xf-yun.com/v1/private/voice_clone`
- **鉴权方式**：标准 hmac-sha256（api_key=APIKey, api_secret=APISecret），与标准TTS相同
- URL 参数：`host`、`date`、`authorization`，**不传 res_id**
- **`res_id` 放 header 里**（不是 URL 参数，也不是 business 里）
- 请求 JSON 结构：
  ```json
  {
    "header": {"app_id": "APPID", "status": 0, "res_id": "assetId"},
    "parameter": {"tts": {"vcn": "x6_clone", "speed": 55}},
    "payload": {"text": {"status": 2, "text": "base64(文本)"}}
  }
  ```
- `vcn`：标准版=x5_clone，多风格版=x6_clone
- **不要传 `aue`/`auf` 参数**（对声音复刻无效，传了会被忽略）
- `speed`: 0-100（50=正常）
- 停顿：文本中插入 `[p500]`（短停顿）或 `[p3000]`（AU打标占位，约3秒）
- 多音字：`着[=zhuo2]手`
- 响应取音频路径：`payload.audio.audio`（base64编码），status=2 时结束
- **输出格式**：裸 PCM（16bit / 24000Hz / 单声道）
- **后处理必备**：加WAV头 → 重采样 24kHz→44.1kHz → 16bit→32bit → 编码 MP3/320kbps

**讯飞声音复刻——已知踩坑记录（必须看！）：**
1. MCP Server `server.py` 中 `_tts_synthesize` 用了错误的 `common/business/data` 格式，需改为 `header/parameter/payload`
2. MCP Server 的 `_get_tts_auth_url` 生成的 URL 缺少 `res_id` 参数（但声音复刻接口的 `res_id` 放 header 里，所以不影响）
3. 合成接口返回的音频数据路径是 `payload.audio.audio`，不是 `data.audio`
4. 输出是裸 PCM 不是 MP3，**不能直接保存为 .mp3**
5. 后处理不能省：PCM→WAV→ffmpeg重采样编码→MP3
6. API 凭证不要写死在文件中（会被系统 auto-redact），用变量在命令行传入

**后处理 Python 代码模板：**
```python
import struct, subprocess, os

def pcm16_to_wav(pcm_bytes):
    datasize = len(pcm_bytes)
    return struct.pack('<4sI4s4sIHHIIHH4sI',
        b'RIFF', 36+datasize, b'WAVE',
        b'fmt ', 16, 1, 1, 24000, 48000, 2, 16,
        b'data', datasize) + pcm_bytes

wav_bytes = pcm16_to_wav(pcm_data)
with open('temp.wav', 'wb') as f: f.write(wav_bytes)
subprocess.run(['ffmpeg', '-y', '-i', 'temp.wav',
    '-ar', '44100', '-sample_fmt', 's32', '-b:a', '320k', 'output.mp3'])
```

**豆包声音复刻（ICL2.0，新版鉴权，推荐）：**
- ⚠ **不是豆包TTS！** 豆包声音复刻和豆包TTS是**两个不同的接口**，不要用 `doubao_tts_synthesize` 工具
- URL: `POST https://openspeech.bytedance.com/api/v3/tts/unidirectional/sse`
- 协议：SSE（流式），非 WebSocket
- 资源ID：`seed-icl-2.0`
- 请求头格式（新版鉴权 X-Api-Key）：
  ```
  X-Api-Key: 8adde342-1e23-4880-8594-0907ddb2c0df
  X-Api-Resource-Id: seed-icl-2.0
  Content-Type: application/json
  ```
- **训练好的音色**：`myvoice_a852e8`（model_type=5, ICL2.0, 新版鉴权）
- 请求体格式：
  ```json
  {
    "user": {"uid": "reasonix"},
    "req_params": {
      "text": "要合成的文本",
      "speaker": "myvoice_a852e8",
      "audio_params": {
        "format": "wav",
        "sample_rate": 24000,
        "speech_rate": 0
      }
    }
  }
  ```
- **speech_rate: 0** 固定默认语速，防止引擎自调
- 输出：24kHz WAV（需重采样到44.1kHz后才能与AU配合使用）
- **没有MCP工具封装**，合成需直接写Python代码

**ICL2.0 Expressive 版完整配置方案（✓已验证通过）：**

### 核心参数

```json
{
  "model": "seed-tts-2.0-expressive",          // ← 必传！开启表现力
  "additions": "{\n" +
    '  "context_texts": ["[情绪承接+本段基调描述]"],\n' +  // ← 每段不同，承接上文
    '  "use_tag_parser": true,\n' +                    // 开启cot标签
    '  "speech_rate": 0,\n' +                          // 固定基准语速
    '  "loudness_rate": 0\n' +                         // 固定基准音量
  "}"
}
```

**注意：** `additions` 必须是 **JSON 字符串**（不是对象），否则 API 报错：`cannot unmarshal object into Go struct field`

### 三明治控制体系

```
第一层：context_texts（每段全局基调）
  → "承接上文的震惊，转为回忆时语速放缓、语气温柔，但带着物是人非的酸楚"
  → 每段都写上承接关系，保证情绪弧线连贯

第二层：speech_rate/loudness_rate（基准参数）
  → 固定0，不做整体调节，把控制权交给cot标签

第三层：cot标签（局部精确微调）
  → <cot text=语速缓慢,语气沉重>内容</cot>
  → 在全局基调上精确控制某一句的语气
```

### cot 标签设计指南

| 段落类型 | cot 写法 | 效果 |
|:---|:---|:---|
| 紧张/震惊 | `<cot text=震惊,难以置信>内容</cot>` | 快进，呼吸急促 |
| 回忆/内心 | `<cot text=语速放慢,语气低沉,回忆>内容</cot>` | 放缓，压低 |
| 压抑/沉重 | `<cot text=语速缓慢,语气沉重>内容</cot>` | 慢速，凝重 |
| 悲伤/痛心 | `<cot text=语速最慢,声音放轻,压抑>内容</cot>` | 最慢，最轻 |
| 愤怒/爆发 | `<cot text=压抑着怒火,语速缓慢但有力>内容</cot>` | 慢而有力 |
| 冷峻决断 | `<cot text=语速缓慢,语气冷峻>内容</cot>` | 字字钉出去 |

**注意事项：**
- 单句text字符长度建议 ≤64（含cot标签）
- cot标签之间不要加空格/换行，连续拼接
- 情感描述用自然语言，越具体越好
- 参考官方示例库风格："用低沉沙哑的语气、带着沧桑与绝望地说"

### 分段合成规则

| 规则 | 说明 |
|:---|:---|
| 每段字数 | 200-500字，在情绪转折处切分 |
| 段间处理 | 1.5秒静音 + 淡入淡出(≥80ms) |
| 每段独立请求 | 避免引擎累积情绪 |
| 语速不均匀 | expressive版会根据文本情绪自调，这是正常行为 |

### 旁白→角色音间隔标准（后期剪辑用）

旁白中原本夹着角色对话的位置，在合成旁白时必须留足间隔，方便后期插入角色音：

| 场景 | 间隔时长 | 说明 |
|:---:|:---:|:---|
| 句号结尾 → 角色对话 | **300-500ms** | 正常对话衔接 |
| 段落结束 → 角色对话 | **500-800ms** | 段落切换 |
| 情绪转折处 → 角色对话 | **800-1200ms** | 重大转折留白 |

**实现方式：** 在cot标签中用"稍作停顿"、"自然衔接"等描述让引擎自然留出间隔，或者在拼接时人为加入对应时长的静音。

### 拼接处理规范（防止哒哒声）

多段PCM拼接时，每段的头尾必须做淡入淡出处理：

| 参数 | 值 | 说明 |
|:---:|:---:|:---|
| 淡入淡出时长 | **≥80ms** | 太短会有"哒哒"杂音 |
| 段间静音 | `\x00` 原生静音 | 不要用空音频文件 |
| 拼接方式 | PCM直接拼接 | 不要解码重编码 |

### 踩坑总结（必读！每一条都是烧钱换来的教训）

#### 流程纪律
1. **永远不能跳步**：必须严格按 Step 0→1→2→3→4 顺序执行，每一步确认后才能进入下一步
2. **方案确认后才能执行**：任何调用API的操作，必须先出方案→等张总说"可以"→再动手。说"听听效果"是让听已有的文件，不是让继续合成
3. **逐行核对文本**：设计cot标签时，必须打开旁白声明清单逐行核对，确保每一行都被分配到合成段中。凭记忆写必然漏字
4. **必须先代入导演角色再分析**：技能包有角色定义，执行时必须先过"自问清单"，不能跳

#### 合成技术
5. **cot标签用自然语言描述**：`<cot text=语速缓慢>内容</cot>`，用导演讲戏的方式写，不要用参数式写法
6. **`additions` 必须是 JSON 字符串**：`'{"use_tag_parser": true}'`，不是对象，否则报错
7. **段间淡入淡出必须做（≥80ms）**：否则拼接处会有"哒哒"声
8. **段间静音用原生静音**：直接用 `\x00` 填充
9. **data块size修复**：SSE返回的WAV data size可能是0xFFFFFFFF，后处理必须修正
10. **输出必须是立体声**：用 `-ac 2` 参数，否则只有左耳有声
11. **MP3拼接不能用concat**：先解码成PCM，拼接处理后再编码回MP3
12. **WAV格式没有比特率参数**：`-b:a` 对WAV无效，WAV转MP3时才用 `-b:a 192k`

#### 文本处理
13. **旁白提取要去掉【角色-CV】"对话"整体**：不能只去标记，否则会漏掉引号内的引用（如"维生素B"）
14. **混合行中标记后的旁白也要提取**：不能只取标记前的部分
15. **分析必须用完整画本**：不能只看提取后的旁白，否则不理解情绪上下文
16. **角色音分析同样用完整画本**：不能只看单独角色的对话

#### 后期制作
17. **夹角色音的位置必须预留间隔**：句号结尾300-500ms，段落结束500-800ms，情绪转折800-1200ms
18. **每次合成前先想后期**：标注夹角色音位置→设计间隔→再写cot标签→再合成

---

## 风险评估与应急预案（必须考虑）

### 已知风险清单

| 风险 | 概率 | 影响 | 应急预案 |
|:---|:---:|:---:|:---|
| **expressive"效果抽卡"** | 中 | 高 | 文档明确写了"存在效果抽卡"。如果某段效果不对，换 standard 版重试，或者调整 context_texts 措辞 |
| **cot标签被静默忽略** | 低 | 高 | 检查 model 是否传了 `seed-tts-2.0-expressive`。standard 版不支持 cot，传了也无效 |
| **段间情绪断裂** | 中 | 中 | 每段独立请求可能导致前后情绪不连贯。通过 context_texts 的三层设计（前情+笔记+指导）来弥补 |
| **上下文太长让引擎混淆** | 低 | 中 | 前情摘要控制在50字以内，不要把整章剧情写进去。引擎只需要"刚才发生了什么"，不需要全部背景 |
| **API额度超限** | 低 | 高 | 后付费模式，注意监控用量。批量合成前先确认额度够用 |
| **文本超长被截断** | 低 | 中 | text字段限制5000字符（含cot标签）。cot标签也占字符数，分段时预留余量 |
| **语音指令测试不达预期** | 中 | 中 | 先试一小段，确认效果再批量。不要一次性合成整章 |
| **音色不一致** | 低 | 高 | 每次请求都传同一个 speaker ID，确保音色统一 |

### 应对策略优先级

```
遇到问题时，按这个顺序排查：

1. 检查 model 是否传了 expressive（最常犯的错误）
2. 检查 additions 是否是 JSON 字符串（第二常犯的错误）
3. 检查 context_texts 措辞是否太简单（"悲伤一点" → "用低沉沙哑的语气..."）
4. 检查 cot 标签格式是否正确（<cot text=描述>内容</cot>）
5. 换 standard 版试试（去掉 model 参数或改成 standard）
6. 如果都不行 → 换引擎（讯飞多风格版 / MiMo）
```

### 最坏情况预案

如果 expressive 版效果始终不理想（抽卡严重），备选方案：

1. **降级到 standard 版**：去掉 model 参数，只用 speech_rate/loudness_rate/pitch，效果稳定但表现力弱
2. **换讯飞多风格版**：用 `vcn=x6_clone` + `rhy` 参数，有5档情感控制，更可预测
3. **换 MiMo VoiceDesign**：导演模式，自然语言控制，不依赖声音复刻

### 重要提醒（容易忽略）

- **standard 版不支持 context_texts 和 cot 标签**：如果没传 model 或传了 standard，这两个参数会被静默忽略，API 不会报错，但效果不对
- **先试后批量**：正式合成前，先拿1-2段测试，确认语气/语速/停顿都对了再批量合成，避免全做完了效果不对
- **cot 标签消耗字符数**：cot 标签本身也计入 text 长度，实际可用文本比看上去少。分段时预留余量
- **context_texts 文本不参与计费**：但长度仍受 API 限制，前情摘要控制在50字以内
- **API Key 安全**：不要写死在代码中，用环境变量传入

**分段合成+拼接代码模板：**
```python
import struct

def fade_inout(pcm_bytes, fade_ms=100, sr=24000):
    fade_samples = int(sr * fade_ms / 1000)
    samples = list(struct.unpack('<' + 'h' * (len(pcm_bytes)//2), pcm_bytes))
    for i in range(min(fade_samples, len(samples))):
        samples[i] = int(samples[i] * (i / fade_samples))
    for i in range(min(fade_samples, len(samples))):
        idx = len(samples) - 1 - i
        samples[idx] = int(samples[idx] * (i / fade_samples))
    return struct.pack('<' + 'h' * len(samples), *samples)

# 合成后拼接
all_pcm = b''
silence = b'\x00' * (24000 * 2 * 1.5)  # 1.5秒静音
for i, seg_text in enumerate(segments):
    wav_data = synthesize(seg_text)  # 调用SSE接口
    pcm = wav_data[44:]  # 去掉WAV头
    pcm = fade_inout(pcm, fade_ms=100)
    all_pcm += pcm
    if i < len(segments) - 1:
        all_pcm += silence

# 生成最终WAV
final_wav = struct.pack('<4sI4s4sIHHIIHH4sI',
    b'RIFF', 36+len(all_pcm), b'WAVE',
    b'fmt ', 16, 1, 1, 24000, 48000, 2, 16, b'data', len(all_pcm)) + all_pcm
```

### 3.3 分段合成策略

| 引擎 | 每段最大字数 | 原因 |
|:---|:---:|:---|
| MiMo VoiceDesign | 300字 | 超过会截断 |
| MiMo VoiceClone | 1200字 | 较宽松 |
| 讯飞声音复刻 | 2000字 | 文本限制 |
| 豆包声音复刻 | 2000字 | API限制 |

### 3.4 输出文件
- 格式：MP3 / 44.1kHz / 32bit / 320kbps（默认，除非用户特别要求）
- 命名格式：`{书名}-{集数:02d}-{CV名}-旁白.mp3`
  - 书名由用户确认，如 `枕边人的毒计`
  - 集数从 01 开始递增
  - CV名由用户指定，如 `墨染青衣Nicou`
  - 示例：`枕边人的毒计-01-墨染青衣Nicou-旁白.mp3`
- 目录：`D:\codexvip\audio-outputs\`
  - `voice-clone/doubao/` — 豆包
  - `voice-clone/xfyun/` — 讯飞
  - `voice-clone/mimo/` — MiMo
  - `tts/mimo/` — MiMo VoiceDesign

### 3.5 AU（Adobe Audition）打标规则

**适用场景：** 旁白与角色音分开录制，后期需要在 AU 中合成的项目。

**核心原理：**
在旁白音频的每个"应该插入角色对话"的位置，植入 **3秒静音**（`[p3000]`）作为占位间隙。合成后检测静音段位置，**自动嵌入 WAV Cue 标记**，AU 打开即可看到标记点。

**具体做法：**

1. **识别打标位置**：对照版文本中，每个 `（|这里打一个标）` 标记的位置，即旁白段落结束、原本有 `【角色-CV】"对话"` 被删除的位置
2. **合成文本植入停顿**：在打标位置插入 `[p3000]`（约3秒静音）
3. **合成后检测静音位置**：用 `ffmpeg silencedetect` 定位每个 `[p3000]` 的精确采样位置
4. **修复data块大小**：原始WAV的data size可能是0xFFFFFFFF（非法值），必须修正为实际数据长度
5. **自动写入WAV标记**：在data块**之后**追加 `cue` 块 + `LIST/adtl` 块（只加labl，不加note，不加_PMX）

**WAV标记结构（AU标准）：**

```
cue 块:    标记数量 + 每个标记{ID, 采样位置, "data", 0, 0, 采样位置}
LIST/adtl: labl子块{ID, 标记名称}（只加labl，不加note）
```

**自动嵌入代码模板（已验证通过）：**
```python
import struct

def add_cue_markers(wav_data, marker_samples, marker_names):
    """修复data size + 在WAV文件末尾追加cue和LIST/adtl标记"""
    pos = 12
    chunks = []
    while pos < len(wav_data) - 8:
        cid = wav_data[pos:pos+4]
        csize = struct.unpack('<I', wav_data[pos+4:pos+8])[0]
        chunk_data = wav_data[pos:pos+8+csize]
        if cid == b'data':
            # 修复data大小
            real_size = len(wav_data) - (pos + 8)
            chunk_data = struct.pack('<4sI', b'data', real_size) + wav_data[pos+8:pos+8+real_size]
            chunks.append(chunk_data)
            break
        chunks.append(chunk_data)
        pos += 8 + csize
        if pos % 2: pos += 1
    
    # 构建cue块
    num = len(marker_samples)
    cue_data = struct.pack('<I', num)
    for i, sp in enumerate(marker_samples):
        cue_data += struct.pack('<II4sIII', i+1, sp, b'data', 0, 0, sp)
    cue_chunk = struct.pack('<4sI', b'cue ', len(cue_data)) + cue_data
    if len(cue_chunk) % 2: cue_chunk += b'\x00'
    
    # 构建LIST/adtl（只加labl，不加note）
    adtl_data = b'adtl'
    for i, name in enumerate(marker_names):
        nb = name.encode('utf-8') + b'\x00'
        labl = struct.pack('<4sI', b'labl', len(nb)+4) + struct.pack('<I', i+1) + nb
        if len(labl) % 2: labl += b'\x00'
        adtl_data += labl
    list_chunk = struct.pack('<4sI', b'LIST', len(adtl_data)) + adtl_data
    if len(list_chunk) % 2: list_chunk += b'\x00'
    
    # 组装：RIFF头 + 原始块 + cue + LIST（cue和LIST在data之后）
    new_wav = b'RIFF' + struct.pack('<I', 0) + b'WAVE'
    for c in chunks:
        new_wav += c
    new_wav += cue_chunk + list_chunk
    new_wav = new_wav[:4] + struct.pack('<I', len(new_wav)-8) + new_wav[8:]
    return new_wav
```

**标记检测命令：**
```bash
ffmpeg -i input.wav -af "silencedetect=noise=-30dB:d=2.5" -f null -
```
从输出中提取 `silence_start` 的时间，乘以采样率得采样位置。

**注意事项：**
- WAV格式：cue + LIST/adtl 是AU标准，无需 `_PMX` 块也能识别
- MP3格式：标记需写入ID3v2的PRIV帧（XMP格式），但推荐直接用WAV
- 标记名称格式：`标记 01`、`标记 02` ...（与张总习惯一致）
- 输出文件名已标注"旁白"，用户一眼区分旁白文件和角色音文件

---

## Step 4：执行合成（代码层面）

调用对应的 MCP 工具执行合成：
- MiMo: `mimo-voiceclone` MCP Server
- 讯飞: `xfyun-voiceclone` MCP Server 或直接 WebSocket
- 豆包: `doubao-voice` MCP Server

合成代码示例（各引擎的具体调用方式见 xfyun-api-credentials 和 voice-tool-priority 记忆文件）

---

## 工具优先级（全局）
- 识图/生图：PaddleOCR-VL → HunyuanOCR → 付费
- 语音/TTS：MiMo → 讯飞 → 豆包
- 声音克隆：MiMo → 讯飞 → 豆包
- 文本对话：DeepSeek V4 Flash（默认）
- 免费优先：所有工具都先用免费额度
