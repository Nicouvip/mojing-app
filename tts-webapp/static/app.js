...
async function generateManualChar(id) {
  ...
  for(var ci=0;ci<customVoices.length;ci++){if(customVoices[ci].id===c.voice){fd.append("custom_voice_b64",customVoices[ci].audioBase64);fd.append("custom_voice_mime",customVoices[ci].mime);break;}}
  fd.append("emotion", "");
  try {
...
async function generateSelectedChapters() {
  ...
  for (const cb of checked) {
    ...
    try {
      ...
      for(var ci=0;ci<customVoices.length;ci++){if(customVoices[ci].id===voice){fd.append("custom_voice_b64",customVoices[ci].audioBase64);fd.append("custom_voice_mime",customVoices[ci].mime);break;}}
...
async function batchGenerateSegments() {
  ...
  for(var ci=0;ci<customVoices.length;ci++){if(customVoices[ci].id===document.getElementById("settingVoice").value){fd.append("custom_voice_b64",customVoices[ci].audioBase64);fd.append("custom_voice_mime",customVoices[ci].mime);break;}}
...
async function cloneVoice() {
  ...
  try {
    ...
    if (result.success) {
      ...
      lastCloneData = { audioBase64: result.audioBase64, mime: "audio/wav" };
