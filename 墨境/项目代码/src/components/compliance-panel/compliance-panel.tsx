...
const SENSE_SVGS: Record<string, React.ReactNode> = {
  eye: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 7C1 7 3 2.5 7 2.5C11 2.5 13 7 13 7C13 7 11 11.5 7 11.5C3 11.5 1 7 1 7Z" />
      <circle cx="7" cy="7" r="2" />
    </svg>),
  hand: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M7 4V2C7 1.5 6.5 1 6 1C5.5 1 5 1.5 5 2V6" />
      <path d="M5 6V3C5 2.5 4.5 2 4 2C3.5 2 3 2.5 3 3V7.5C3 9.5 4.5 12 7 12H9C10.5 12 12 10.5 12 9V7.5C12 6.5 11.5 6 10.5 6C10 6 9.5 6.5 9.5 7" />
    </svg>),
  ear: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M10 5C10 2.5 8.5 1 6.5 1C4.5 1 3 2.5 3 5C3 7 4.5 8 5 8.5C5.5 9 5.5 10 5.5 11" />
      <path d="M5.5 11C5.5 11.5 6 12 6.5 12C7 12 7.5 11.5 7.5 11" />
      <path d="M8 7.5C9 7 10 6 10 5" />
    </svg>),
  nose: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4.5 9.5C4.5 9.5 7 11.5 9.5 9.5" />
      <path d="M5 9C4.5 7 6 4 7 3C8 4 9.5 7 9 9" />
    </svg>),
  mouth: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 8.5C3 8.5 5 11.5 7 11.5C9 11.5 11 8.5 11 8.5" />
    </svg>),
}
...
const CATEGORY_ICONS: Record<string, React.ReactNode> = {
  ...
  compliance: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M10 2.5L16 5V10C16 13.5 13.5 16.5 10 18C6.5 16.5 4 13.5 4 10V5L10 2.5Z" />
      <path d="M7.5 10.5L9 12L12.5 8.5" />
    </svg>),
  engine: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 15.5L5 14" /><path d="M15 5L16 4.5" />
      <path d="M8 5L9 7" /><path d="M12 5L11 7" />
      <path d="M5 8L7 9" /><path d="M5 12L7 11" />
      <path d="M15 8L13 9" /><path d="M15 12L13 11" />
      <circle cx="10" cy="10" r="6.5" />
      <path d="M10 6V10.5L13 12" />
    </svg>),
  story: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 3.5H10V16.5H4C3.5 16.5 3 16 3 15.5V4.5C3 4 3.5 3.5 4 3.5Z" />
      <path d="M10 3.5H16C16.5 3.5 17 4 17 4.5V15.5C17 16 16.5 16.5 16 16.5H10V3.5Z" />
      <path d="M10 4V16" />
    </svg>),
}
    ...
    <>
      ...
      {!open && (
        <button
          ...
          style={{ boxShadow: '-2px 0 12px rgba(196,149,106,0.2)' }}
          aria-label="展开合规面板"
        >
      ...
      >
        <div className="flex h-full w-[300px] flex-col overflow-y-auto px-4 py-5">
          ...
          <CategorySection
            ...
            iconClass="red"
            title="合规检测"
            badge={`${compliance.classB.violations + (compliance.classA.count > compliance.classA.max ? 1 : 0) + compliance.postActionExplain.count}`}
            badgeWarn
          >
            ...
            <div className="mb-2 flex flex-wrap gap-1 text-xs text-[#1a1814]">
              {engine.scenes.map(s => (
                <span key={s.id} className={`inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 ${
                  ...
                  s.status === 'cooling' ? 'opacity-50' : 'opacity-35'
                }`}>
          ...
          <CategorySection icon={CATEGORY_ICONS.story} iconClass="green" title="伏笔与角色" badge={`${story.activeForeshadowing.length}条`}>
            <DetailRow
              label="活跃伏笔"
              value={story.activeForeshadowing.length > 0
                ? story.activeForeshadowing.map(f => `${f.name}`).join(' · ')
                : '无活跃伏笔'
              }
            />
            <DetailRow
              label="角色在场"
              value={`${story.charactersInScene}/${story.totalCharacters}`}
            />
          </CategorySection>
        </div>
      </div>
...
function CategorySection({
  icon, iconClass, title, badge, badgeWarn, children
}: {
  ...
  return (
    <div className="mb-0.5 overflow-hidden rounded-xl transition-all duration-200">
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 transition-colors duration-200 hover:bg-[rgba(196,149,106,0.06)]"
        aria-expanded={open}
      >
        <div className="flex items-center gap-2">
          <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-sm ${
            ...
            iconClass === 'red' ? 'bg-[rgba(176,96,96,0.1)] text-[#b06060]' :
            iconClass === 'purple' ? 'bg-[rgba(139,92,246,0.1)] text-[#8b5cf6]' :
            'bg-[rgba(106,138,106,0.1)] text-[#6a8a6a]'
          }`}>
            {icon}
          </div>
        ...
        <div className="h-1 w-14 overflow-hidden rounded-full bg-[rgba(26,24,20,0.06)]">
          <div
            ...
            style={{
              ...
              background: color === 'gold' ? '#c4956a' : color === 'warn' ? '#b8a060' : '#6a8a6a',
            }}
          />
        </div>
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-[5px] text-xs">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className={`inline-block h-1.5 w-1.5 rounded-full ${isPass ? 'bg-[#6a8a6a]' : 'bg-[#b06060]'}`} />
      </div>
    </div>
  )
}
