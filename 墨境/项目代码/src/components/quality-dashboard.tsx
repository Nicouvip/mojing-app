  ...
  return (
    <div className="space-y-8">
      ...
      {goals.enabled && goalPct > 0 && (
        <div className="bg-white rounded-2xl p-4 border border-border/50 shadow-sm flex items-center gap-4">
          ...
          {streak > 0 && <div className="text-right shrink-0"><div className="text-lg font-bold text-amber-600">🔥 {streak}</div><div className="text-[10px] text-muted-foreground">连续写作</div></div>}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <SummaryCard
          ...
          label="平均评分"
          value={`${avgScore}/5`}
          color={avgScore >= 4 ? 'text-emerald-600' : avgScore >= 3 ? 'text-amber-600' : 'text-destructive'}
        />
        <SummaryCard
          ...
          color={complianceRate >= 80 ? 'text-emerald-600' : complianceRate >= 50 ? 'text-amber-600' : 'text-destructive'}
        />
        <SummaryCard
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-5 border border-border/50 shadow-sm">
          ...
          <SimpleLineChart data={scoreData} color="#10b981" />
        ...
        <div className="bg-white rounded-2xl p-5 border border-border/50 shadow-sm">
          ...
          <div className="relative">
            <SimpleLineChart data={densityData} color="#f59e0b" />
            <div className="absolute inset-x-0 top-[28%] h-[34%] border-y border-dashed border-emerald-400/40 pointer-events-none" />
          </div>
          <div className="flex justify-between text-[10px] text-muted-foreground mt-1">
            <span>目标区 40-55%</span>
        ...
        <div className="bg-white rounded-2xl p-5 border border-border/50 shadow-sm">
          ...
          <SimpleBarChart data={wordCountData} color="#3b82f6" />
        </div>
      </div>
            ...
            {allProjects.map(p => (
              <Link key={p.id} href={'/quality-check/' + p.id}
                ...
                <div className="flex gap-3 mt-2 text-xs">
                  ...
                  <span className={p.quality!.complianceRate >= 80 ? 'text-emerald-600' : 'text-amber-600'}>合规 {p.quality!.complianceRate}%</span>
                </div>
              </Link>
            ))}
          </div>
      ...
      {selected && (
        <div className="bg-white rounded-2xl p-5 border border-border/50 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <MiniStat label="55字线" value={selected.openingHook ? '通过' : '未通过'} color={selected.openingHook ? 'text-emerald-600' : 'text-destructive'} />
          </div>
...
function SummaryCard({ icon, label, value, sub, color }: {
  ...
  return (
    <div className="rounded-2xl p-4 border border-border/50 shadow-sm bg-white">
...
function getScoreColor(score: number): string {
  return score >= 4 ? 'text-emerald-600' : score >= 3 ? 'text-amber-600' : 'text-destructive'
}
